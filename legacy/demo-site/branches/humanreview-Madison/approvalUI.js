...
import { executeTripManually, checkExecutionEligibility } from 'backend/tripService';
...
export function displayExecutionUI(trip, uiElements) {
    ...
    checkExecutionEligibility(trip)
        .then(response => {
            if (response.success) {
...
export function handleExecuteTrip(trip, action = "SEND_BOOKING_REQUEST", notes = "", onSuccess, onError) {
    ...
    // Execute trip
    executeTripManually(trip.trip_id, trip, action, "human")
        .then(response => {
            if (response.success) {
                ...
                if (onError) {
                    ...
                    displayExecutionFailure(response.data || { error: errorMsg });
                }
            }
        })
        .catch(error => {
