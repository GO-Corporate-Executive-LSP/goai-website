...
function evaluateCondition(condition, trip) {
  ...
  if (condition.includes('state === "failed"')) {
    return trip.state.current_state === 'failed';
  }
  if (condition.includes('retry_count > 0')) {
    return trip.admin_context.retries.retry_count > 0;
  }
  if (condition.includes('state === "escalated"') || condition.includes('state === "pending_approval"')) {
    return trip.state.current_state === 'escalated' || trip.state.current_state === 'pending_approval';
  }
  return true;
}
...
export function filterTripsForQueue(trips, filters = {}) {
  ...
  // Filter by state
  if (filters.state) {
    filtered = filtered.filter(trip => trip.state.current_state === filters.state);
  }
  
  // Filter by failure type
  if (filters.failure_type) {
    filtered = filtered.filter(trip => trip.admin_context.failure.failure_type === filters.failure_type);
  }
  
  // Filter by tier
  if (filters.tier) {
    filtered = filtered.filter(trip => trip.tier.name === filters.tier);
  }
  ...
  if (filters.retry_count_gt !== undefined) {
    filtered = filtered.filter(trip => trip.admin_context.retries.retry_count > filters.retry_count_gt);
  }
  ...
  if (filters.time_in_state_gt) {
    ...
    filtered = filtered.filter(trip => {
      const stateTime = new Date(trip.state.state_changed_at).getTime();
  ...
  if (filters.sentinel_flag) {
    filtered = filtered.filter(trip => trip.sentinel_snapshot.sentinel_flag === filters.sentinel_flag);
  }
...
export function sortTripsForQueue(trips) {
  return trips.sort((a, b) => {
    // 1. Sort by priority
    const priorityA = getPriorityForState(a.state.current_state);
    const priorityB = getPriorityForState(b.state.current_state);
    const priorityOrder = {
    ...
    const timeA = new Date(a.state.state_changed_at).getTime();
    const timeB = new Date(b.state.state_changed_at).getTime();
    if (timeA !== timeB) {
      return timeA - timeB;
    }
    
    // 3. Sort by retry count (most retries first)
    if (a.admin_context.retries.retry_count !== b.admin_context.retries.retry_count) {
      return b.admin_context.retries.retry_count - a.admin_context.retries.retry_count;
    }
    
    // 4. Sort by created date (oldest first)
    const createdA = new Date(a.admin_context.created_at).getTime();
    const createdB = new Date(b.admin_context.created_at).getTime();
