...
export async function addToAdminQueue(trip) {
  try {
    ...
    logError({
      type: "DB_ERROR",
      action: "add_to_admin_queue",
      tripId: trip?.trip_id || "",
      state: trip?.state?.current_state || ""
    }, error);
    logAuditError(trip.trip_id, error, { });
    throw error;
  }
...
export async function logAuditEvent(eventData) {
  try {
    ...
    logEvent({
      type: "AUDIT_EVENT",
      tripId: auditEntry.tripId || "",
      from: auditEntry.fromState || null,
      to: auditEntry.toState || null,
      eventType: auditEntry.event?.event_type || "",
      eventCategory: auditEntry.event?.event_category || "",
