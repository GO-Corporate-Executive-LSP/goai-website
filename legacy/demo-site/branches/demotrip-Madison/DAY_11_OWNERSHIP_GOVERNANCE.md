...
- Temporary network issue → SHORT_BACKOFF (30s)
...
|-------|---------|---------|
| **Trip ID** | Unique identifier | TRP_12345 |
| **Priority** | Urgency level | 🔴 HIGH |
| **State** | Current state | pending_approval |
| **Time in State** | How long waiting | 8 minutes |
| **SLA Status** | Time remaining | ⚠️ 7 min left |
| **User** | Who requested | john@example.com |
...
|-------|-------------------|--------------|
| **pending_approval** | Approve, Request Adjustment, Reject, Escalate | Notes required for Reject/Escalate |
| **failed** | Retry Manually, Contact User, Cancel | Must document reason |
| **needs_adjustment** | View User Response, Approve, Reject | User must submit changes first |
| **escalated** | Approve Override, Reject, Contact User | Senior admin only, full notes required |
| **in_progress** | Contact User, Monitor, View Tracking | No state change allowed |
...
|--------|------------|----------|--------------|
| **Approve** | pending_approval | approved | Triggers Lee execution |
| **Request Adjustment** | pending_approval | needs_adjustment | User notified, returns to draft |
| **Reject** | pending_approval | cancelled | User notified, no charge |
| **Escalate** | pending_approval | escalated | Senior admin notified |
...
| **Decision Made** | Admin ID, trip ID, action, timestamp, from_state, to_state | Notes |
...
| **Escalation** | Admin ID, trip ID, reason, senior_admin, timestamp | Priority change |
...
| **Enriches trip objects** | Adds assessment data asynchronously | Trip object includes sentinel_enrichment field |
...
1. **[tripValidation.js](../src/backend/etas/tripValidation.js)** — Validation logic (Day 3)
2. **[tierDefinitions.js](../src/backend/etas/tierDefinitions.js)** — Tier rules (Day 4)
3. **[humanReviewRules.js](../src/backend/etas/humanReviewRules.js)** — Review triggers (Day 5)
4. **[rolesPermissions.js](../src/backend/etas/rolesPermissions.js)** — Permissions (Day 6)
5. **[failureHandling.js](../src/backend/etas/failureHandling.js)** — Failure handling (Day 7)
6. **[uxMessaging.js](../src/backend/etas/uxMessaging.js)** — User messaging (Day 8)
7. **[adminSurfaces.js](../src/backend/etas/adminSurfaces.js)** — Admin controls (Day 9)
8. **[DAY_10_TESTING_ARTIFACTS.md](DAY_10_TESTING_ARTIFACTS.md)** — Testing scenarios (Day 10)
