...
Description:
Trip is review-ready and awaiting human approval.
...
- Show SENTINEL context (if available)
- Present approval options

Human Approval Options:
- ✅ APPROVED → proceed to execution (future)
- ✏️ NEEDS_ADJUSTMENT → return to conversation
- 🧍 ESCALATED → route to concierge

Transitions:
- → APPROVED (on human approval)
- → NEEDS_ADJUSTMENT (return to draft)
...
## STATE: APPROVED (Day 9)
Description:
Trip has been approved by human decision-maker.

Trip State:
- approved

System Actions:
- Log approval decision with audit trail
- Display "Ready for execution" message
- No execution yet (Day 9)

This is a terminal state for Day 9.
Future milestones will enable execution from this state.

---

## STATE: NEEDS_ADJUSTMENT (Day 9)
Description:
Human has requested changes to the trip.
...
Transitions:
- → INTENT_CAPTURE (restart conversation)
- → Any modification state based on what needs adjustment

---

## STATE: ESCALATED (Day 9)
Description:
Trip has been escalated to human concierge.
...
- Display concierge contact options
- Show Calendly / SMS workflow
