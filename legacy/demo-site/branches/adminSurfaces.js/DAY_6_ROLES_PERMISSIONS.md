...
- ✅ **Who can do this action?**  
  → Check PERMISSION_MATRIX for role, STATE_PERMISSIONS for state-specific rules

- ✅ **In which state?**  
  → Check STATE_PERMISSIONS for allowed actions per state per role

- ✅ **What happens if they try anyway?**  
  → `canPerformAction()` returns `{ allowed: false, reason, code }`

- ✅ **Who has final authority?**  
  → APPROVAL_AUTHORITY: Users REQUEST, Admins APPROVE, System ENFORCES

---

## 📂 Files Created

1. **[rolesPermissions.js](../src/backend/etas/rolesPermissions.js)** — Complete permission system:
